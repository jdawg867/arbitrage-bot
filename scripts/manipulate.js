...
const POOL_FEE = config.MANIPULATE.POOL_FEE

async function main() {
  // Fetch contracts (TOKEN_IN is the token we sell to push the price)
  const {
    token0: ARB_AGAINST,
    token1: ARB_FOR
  } = await getTokenAndContract(config.MANIPULATE.TOKEN_IN, config.MANIPULATE.TOKEN_OUT, provider)

  const pool = await getPoolContract(EXCHANGE_TO_USE, ARB_AGAINST.address, ARB_FOR.address, POOL_FEE, provider)
